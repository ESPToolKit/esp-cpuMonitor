...
template <typename TResult> static bool idleHookRegisteredSuccessfully(const TResult &res) {
#if defined(ESP_OK)
...
template <typename TResult> static int idleHookErrorCode(const TResult &res) {
#if defined(ESP_OK)
	if constexpr (std::is_same<TResult, esp_err_t>::value) {
...
static bool isValidSmoothingMode(CpuSmoothingMode mode) {
	return mode == CpuSmoothingMode::None || mode == CpuSmoothingMode::RollingMean ||
...
void ESPCpuMonitor::resetState(const CpuMonitorConfig &cfg) {
	...
	history_ = CpuMonitorDeque<CpuUsageSample>(
	    CpuMonitorAllocator<CpuUsageSample>(config_.usePSRAMBuffers)
	);
	callbacks_ = CpuMonitorVector<CpuSampleCallback>(
	    CpuMonitorAllocator<CpuSampleCallback>(config_.usePSRAMBuffers)
	);
	resetSmoothingState();
...
bool ESPCpuMonitor::init(const CpuMonitorConfig &cfg) {
	...
	ESP_LOGI(
	    TAG,
	    "ESPCpuMonitor started (period=%ums, calibration=%u)",
...
void ESPCpuMonitor::deinit() {
	...
	CpuMonitorDeque<CpuUsageSample>(CpuMonitorAllocator<CpuUsageSample>(config_.usePSRAMBuffers))
	    .swap(history_);
	CpuMonitorVector<CpuSampleCallback>(
	    CpuMonitorAllocator<CpuSampleCallback>(config_.usePSRAMBuffers)
	)
	    .swap(callbacks_);
	resetSmoothingState();
...
bool ESPCpuMonitor::sampleNow(CpuUsageSample &out) {
	...
	CpuMonitorVector<CpuSampleCallback> callbacks{
	    CpuMonitorAllocator<CpuSampleCallback>(config_.usePSRAMBuffers)
	};
...
void ESPCpuMonitor::timerCallback(void *arg) {
	...
	CpuUsageSample sample{};
	CpuMonitorVector<CpuSampleCallback> callbacks{
	    CpuMonitorAllocator<CpuSampleCallback>(self->config_.usePSRAMBuffers)
	};
...
bool ESPCpuMonitor::captureSample(
    CpuUsageSample &out, CpuMonitorVector<CpuSampleCallback> &callbacksCopy
) {
	bool ready = false;
	lock();
...
bool ESPCpuMonitor::computeSampleLocked(CpuUsageSample &out) {
	...
	if (!calibrated_) {
		...
		if (calibrationSamplesDone_ >= calibrationSamplesNeeded_) {
			...
			ESP_LOGI(
			    TAG,
			    "CPU usage baseline calibrated after %u samples",
			    calibrationSamplesNeeded_
			);
		}
		return false;
	...
	if (readTemperature(temperature)) {
		if (temperatureSamples_ == 0) {
			...
			temperatureAvg_ +=
			    (temperature - temperatureAvg_) / static_cast<float>(temperatureSamples_ + 1);
		}
		temperatureSamples_++;
